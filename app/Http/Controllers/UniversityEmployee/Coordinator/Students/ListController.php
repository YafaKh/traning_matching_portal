...
{
   /* $request->validate([
        'list' => 'required|extension:xlsx,xls'
    ]);
    dd($request->hasFile('list'));
    if(($request->hasFile('list')))
    {
        Facades\File::delete('assets/excel/registration_list');
        $request->list->move(public_path('assets/excel'), 'registration_list.xlsx');
    }*/
    $request->validate([
